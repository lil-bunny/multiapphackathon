...
def send_driver_details_ack(state: WorkflowState) -> WorkflowState:
    crm_row = state.data.get("crm_row")
    if not isinstance(crm_row, dict):
        state.data["workflow_error"] = "missing_crm_row_for_ack"
        return state
    ...
    result = CarrierAckService().send_driver_details_thanks(
    ...
    state.data["carrier_ack_sent"] = bool(result.get("success"))
    if not result.get("success"):
        state.data["carrier_ack_error"] = str(result.get("error") or "send_failed")
    logger.info(
        "carrier ack shipment_id=%s sent=%s",
        crm_row.get("shipment_id"),
        state.data["carrier_ack_sent"],
    )
    return state
